...
function App() {
  ...
  const narrativeStates = [
    ...
    },
    {
      ...
      text: "Trained on image-text pairs, neural nets encode prompts into latent vectors. Then, they decode these into images by adjusting pixels to fit the learned patterns.",
      state: 'word_to_picture'
    },
    {
  ...
  const createThreeJSApp = (): ThreeJSApp => {
    ...
    const sceneInitializers: { [key: string]: () => any[] | Promise<any[]> } = {
      ...
      'word_to_picture': () => {
        // Word-to-Vector Flow Animation
        const group = new window.THREE.Group()
        const particles = []
        const words = ['cat', 'tree', 'house', 'river', 'mountain', 'sun', 'bird', 'flower', 'ocean', 'forest']
        const vectorPoints = []

        // Create text particles flowing from left to right
        for (let i = 0; i < 50; i++) {
          const geometry = new window.THREE.SphereGeometry(0.02, 8, 8)
          const material = new window.THREE.MeshBasicMaterial({
            color: new window.THREE.Color().setHSL(0.55, 0.8, 0.6), // Cyan color matching neural network theme
            transparent: true,
            opacity: 0.8
          })
        ...
        // Create vector coordinate points on the right side
        for (let i = 0; i < 30; i++) {
          const geometry = new window.THREE.SphereGeometry(0.03, 8, 8)
          const material = new window.THREE.MeshBasicMaterial({
            color: new window.THREE.Color().setHSL(0.55, 1.0, 0.8), // Brighter cyan for vector points
            transparent: true,
            opacity: 0.9
          })

          const vectorPoint = new window.THREE.Mesh(geometry, material)

          // Position vector points on the right side in a 3D grid pattern
          const gridSize = 3
          const x = 4 + (i % gridSize) * 0.8 - (gridSize * 0.8) / 2
          const y = ((i / gridSize) % gridSize) * 0.8 - (gridSize * 0.8) / 2
          const z = Math.floor(i / (gridSize * gridSize)) * 0.8 - 0.8

          vectorPoint.position.set(x, y, z)
          vectorPoint.userData = {
            originalPosition: vectorPoint.position.clone(),
            pulsePhase: Math.random() * Math.PI * 2,
            connections: []
          }

          vectorPoints.push(vectorPoint)
          group.add(vectorPoint)
        }

        // Create connection lines between particles and vector points
        const connectionGeometry = new window.THREE.BufferGeometry()
      ...
      switch(animationState) {
        ...
        case 'word_to_picture':
          if (objects[0]) {
            ...
            // Animate word particles flowing from left to right
            userData.particles.forEach((particle: any) => {
              // Move particles to the right
              particle.position.x += particle.userData.speed

              // Add some vertical oscillation
              particle.position.y += Math.sin(time * 2 + particle.userData.phase) * 0.005

              // Reset particles that go too far right, creating continuous flow
              ...
              // Pulse opacity based on position (fade in from left, fade out toward right)
              const progress = (particle.position.x + 8) / 18 // 0 to 1 as particle moves from -8 to 10
              particle.material.opacity = 0.8 * Math.min(progress * 2, (1 - progress) * 2, 1)
            })

            // Animate vector points with pulsing
            userData.vectorPoints.forEach((vectorPoint: any) => {
              const pulse = Math.sin(time * 3 + vectorPoint.userData.pulsePhase) * 0.1 + 0.9
              vectorPoint.scale.setScalar(pulse)

              // Add subtle position oscillation
              vectorPoint.position.x = vectorPoint.userData.originalPosition.x +
                Math.sin(time * 2 + vectorPoint.userData.pulsePhase) * 0.05
              vectorPoint.position.y = vectorPoint.userData.originalPosition.y +
                Math.cos(time * 1.5 + vectorPoint.userData.pulsePhase) * 0.03
            })

            // Update connection lines between nearby particles and vector points
            const connectionPositions = userData.connectionPositions
            const connectionColors = userData.connectionColors
            let connectionIndex = 0

            userData.particles.forEach((particle: any) => {
              userData.vectorPoints.forEach((vectorPoint: any) => {
                const distance = particle.position.distanceTo(vectorPoint.position)

                // Create connections for particles within range of vector points
                if (distance < 3 && connectionIndex < connectionPositions.length / 6) {
                  // Set line positions
                  connectionPositions[connectionIndex * 6] = particle.position.x
                  connectionPositions[connectionIndex * 6 + 1] = particle.position.y
                  connectionPositions[connectionIndex * 6 + 2] = particle.position.z
                  connectionPositions[connectionIndex * 6 + 3] = vectorPoint.position.x
                  connectionPositions[connectionIndex * 6 + 4] = vectorPoint.position.y
                  connectionPositions[connectionIndex * 6 + 5] = vectorPoint.position.z

                  // Set line colors (gradient from particle to vector point)
                  const intensity = Math.max(0, 1 - distance / 3)
                  for (let c = 0; c < 6; c += 3) {
                    connectionColors[connectionIndex * 6 + c] = 0.2 + intensity * 0.3     // R
                    connectionColors[connectionIndex * 6 + c + 1] = 0.6 + intensity * 0.4 // G
                    connectionColors[connectionIndex * 6 + c + 2] = 0.8 + intensity * 0.2 // B (cyan gradient)
                  }
            ...
            // Mark geometries for update
            userData.connections.geometry.attributes.position.needsUpdate = true
            userData.connections.geometry.attributes.color.needsUpdate = true
          }
          break
        case 'noise':
          if (objects[0]) {
